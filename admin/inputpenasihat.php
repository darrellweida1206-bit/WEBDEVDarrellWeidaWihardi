...
if(isset($_POST['Simpan'])) 
{
    ...
    $penasihat_FILE = ""; // Set default kosong dulu
    
    // Cek apakah ada file yang diupload
    if(!empty($_FILES['penasihatFILE']['name'])) {
        $file_name = $_FILES['penasihatFILE']['name'];
        $dokumen_tmp = $_FILES['penasihatFILE']['tmp_name'];
        
        $typefoto = pathinfo($file_name, PATHINFO_EXTENSION);
        ...
        if($ukuranfoto <= 5000000){
            if(($typefoto=="jpg") or ($typefoto=='png')){
                if(!file_exists($path)){
                    move_uploaded_file($dokumen_tmp, 'images/' . $file_name);
                }
                $penasihat_FILE = $file_name; // Hanya simpan nama jika jpg/png
            }
            // Jika bukan jpg/png, $penasihat_FILE tetap kosong
        }
    }
...
<div class="row mb-3">
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="penasihatKET" name="penasihatKET" placeholder="Isikan Keterangan">
    </div>
  </div>
