...
{
    if(isset($_REQUEST['idJadwal']))
    {
        $jadwal_ID = $_REQUEST['idJadwal'];
    }
    if(empty($jadwal_ID))
    {
        ?></h1>Maaf anda salah input</><?php
        die("anda harus mengisi ID Jadwal");
    }
    ...
    mysqli_query($conn, "insert into jadwalwisuda values('$jadwal_ID', '$jadwal_Periode', '$jadwal_Tanggal', '$jadwal_Tempat', '$wisuda_FOTO')"); /* memasukkan data ke dalam tabel */
...
{
    $query = mysqli_query($conn, "select * from jadwalwisuda"); /* mengambil data dari tabel jadwal */
}
/** end pencarian data */
?>
  ...
  <div class="row mb-3 mt-5">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="idJadwal" name="idJadwal" placeholder="ID Jadwal Wisuda" maxlength="11" required="">
    </div>
  </div>
  <div class="row mb-3">
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="tempatWisuda" name="tempatWisuda" placeholder="Tempat Pelaksanaan Wisuda" maxlength="100">
    </div>
  </div>
  <div class="row mb-3">
